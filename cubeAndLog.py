...
def run_sequence(logger, sequence, base_x, base_y, base_z, yaw):
    cf = logger._cf
    ...
    for position in sequence:
        ...
        for i in range(30):
            ...
            with open('testCSV/' + str(logger.droneNum)+ "/theory.csv",'a') as fd:
...
if __name__ == '__main__':
    ...
    if len(available) > 0:
        ...
        f = open("./testCSV/" + str(le.droneNum) + "/theory.csv", 'w')
        f.write("x, y, z \n")
        f.close()
        run_sequence(le, sequence, initial_x, initial_y, initial_z, initial_yaw)
